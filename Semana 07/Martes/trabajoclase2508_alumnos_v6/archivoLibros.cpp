...
bool ArchivoLibros::mostrarRegistroLibro(int estado)
{
...
int ArchivoLibros::cantidadRegistros()
{
    FILE *pLibro;
    pLibro = fopen(_nombreArchivo, "rb");
    ...
    fseek(pLibro, 0,SEEK_END);  // movemos el puntero al final del archivo
    int totalBytes = ftell(pLibro); // se le asigna a la variable el peso total del archivo
    int tamanioRegistro = totalBytes/sizeof(Libro);
    fclose(pLibro);
